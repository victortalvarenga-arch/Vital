...
relatorios.get('/resumo', (req, res) => {
  ...
  const porProfissional = db.prepare(
    `SELECT p.id, p.nome, p.comissao, COUNT(*) qtd, SUM(a.valor) producao
       FROM appointments a JOIN staff p ON p.id=a.staff_id
      WHERE a.data LIKE ? AND a.status='concluido'
      GROUP BY p.id ORDER BY producao DESC`
  ).all(like).map(r => ({ ...r, comissaoValor: (r.producao || 0) * (r.comissao || 0) / 100 }));
...
relatorios.get('/ocupacao', (req, res) => {
