...
- **Site — Clínica:** same pills, different voice — every `.b` goes 11px / 700 / `.15em` / uppercase, inside the booking window too, and lifts `translateY(-2px)` on hover (`.18s ease`) instead of only shifting color. One extra variant, **gold** (`.clinica-cta-ouro`: `--ouro-claro` fill, `--texto` on top, `brightness(1.04)` on hover), used in exactly two places: the hero's "Agende seu horário" and the WhatsApp FAB. Every other filled button — the top bar's "Agendar", the Agende card's CTA, "Seguir no Instagram", the service pills — is the ordinary `.b-p` in the tenant's `--marca-escura`. Beside the gold CTA sits a **text link** (`.clinica-cta-link`, "Conheça os serviços", same uppercase voice, inherits the band's white, gold on hover), so the hero offers one action and one glance, not two buttons. The three identical "Ver opções" pills under the category circles are hidden on this model (`.svc-btn-cat`): the circle is already the button and the name sits under it; the "Agendar" pill on the *service* grid stays because there it is the CTA.
...
- **Do** keep Clínica's sections honest before they are pretty: Avaliações renders its world-text empty state until a real review exists, Antes/Depois only shows an authorized real case, Equipe comes from real staff, Instagram links out instead of faking a grid. The reference had testimonials, a "5,0" and "+8 anos"; none of it was carried over, and none of it may be added as placeholder.
